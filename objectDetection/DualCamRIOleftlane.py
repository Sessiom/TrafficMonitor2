...
def process_camera(esp32_ip):
    ...
    roi_top = 0.5  # Start from 70% of the image height
    roi_left = 0.4  # Start from the middle of the image
    roi_bottom = 1.0  # End at the bottom of the image
    roi_right = 1.0  # End at the right of the image
    ...
    while True:
        ...
        results = model(img, stream=True)  #(img needs to be imgRegion for mask)
        for r in results:
            boxes = r.boxes
            for box in boxes:
